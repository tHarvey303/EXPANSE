...
import numpy as np
filters = ['F090W', 'F115W', 'F150W','F162M', 'F182M', 'F200W', 'F210M', 'F250M', 'F277W', 'F300M', 'F335M', 'F356W', 'F410M', 'F430M']
...
flux_f444w = np.array([aperture.do_photometry(psf_f444w)[0] for aperture in [CircularAperture(center, r=radius) for radius in radii]])
...
for i, filter in enumerate(filters):
    ...
    ax[0].plot(radii_arcsec, encircled_energy_before, label=filter, color=cmap(i/len(filters)))
    ax[1].plot(radii_arcsec, encircled_energy_after, label=filter, color=cmap(i/len(filters)))
