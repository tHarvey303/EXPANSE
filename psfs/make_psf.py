...
def make_psf(
    ...
    for pfilt in use_filters:
        ...
        if not skip_psf:
            ...
            psf.measure()
            psf.select(
                snr_lim=snr_lim, dshift_lim=3.5, mask_lim=0.99, showme=showme, nsig=30
            )
            print(f"First selection {np.sum(psf.ok)} stars left")
            psf.stack(sigma=sigma)  # Moved from between selects

            psf.select(
                snr_lim=snr_lim, dshift_lim=3.5, mask_lim=0.6, showme=True, nsig=30
            )
            ...
            show_cogs(
                [psf.psf_average], title=pfilt, label=["oPSF"], outname=plotdir + pfilt
            )
        ...
        if method == "pypher":
            ...
            fits.writeto(
                kernel_dir + "psf_b.fits", target_psf, header=hdr, overwrite=True
            )
        ...
        fits.writeto(
            outname, np.float32(np.array(kernel / kernel.sum())), overwrite=True
        )
    ...
    for i, pfilt in enumerate(use_filters[1:]):
        ...
        plt.imshow(
            filt_psf_conv, norm=simple, interpolation="antialiased", origin="lower"
        )
...
def imshow(args, cross_hairs=False, log=False, **kwargs):
    ...
    fig, ax = plt.subplots(
        nrows=nrow, ncols=ncol, figsize=(ncol * panel_width, nrow * panel_width)
    )
...
def find_stars(
    ...
    if showme:
        ...
        plt.savefig(
            outdir + "/" + os.path.basename(filename).replace(suffix, "_diagnostic.pdf")
        )
...
class PSF:
    ...
    def center(self, window=21, interpolation=cv2.INTER_CUBIC):
        ...
        self.cat = hstack(
            [self.cat, Table(np.array(pos), names=["x0", "y0", "x1", "y1", "dshift"])]
        )
    ...
    def save(self, outname=""):
        ...
        fits.writeto(
            "_".join([outname, "psf.fits"]), np.array(self.psf_average), overwrite=True
        )
    ...
    for band in bands:
        ...
        if band != match_band:
            ...
            if overwrite or not os.path.exists(outwhtname):
                ...
                err_conv = convolve_func(err, kernel, **convolve_kwargs).astype(
                    np.float32
                )
...
def psf_comparison(
    ...
    for i, band in enumerate(bands):
        ...
        if i == len(bands) - 1:
            axs[i].legend(
                frameon=False, fontsize=7, loc="lower right", bbox_to_anchor=(1.0, 0.13)
            )
            axs[i].text(
                0.17,
                0.84,
...
# Compute COG for PSF
def psf_cog(
    psfmodel, filt, nearrad=None, fix_extrapolation=True, pixel_scale=None, norm_rad=1.0
):
...
def get_webbpsf(
    ...
    PATH_SW_ENERGY = (
        "/nvme/scratch/work/tharvey/aperpy/config/Encircled_Energy_SW_ETCv2.txt"
    )
    PATH_LW_ENERGY = (
        "/nvme/scratch/work/tharvey/aperpy/config/Encircled_Energy_LW_ETCv2.txt"
    )
    ...
    outname = os.path.join(
        output, "psf_" + field + "_" + filt + "_" + str(fov) + "arcsec_" + str(angle)
    )  # what to save as?
...
def psf_correction_factor(
    match_band, psf_dir, apersize=0.32, pixel_scale=0.03, fix_extrapolation=False
):
...
if __name__ == "__main__":
    ...
    print(folders)
    im_paths = {
        band: [glob.glob(f"{folder}/*{band.lower()}*.fits")[0] for folder in folders]
        for band in bands
    }
    ...
    for band in ["F850LP", "F814W", "F775W", "F606W", "F435W"]:
        ...
        band_path = (
            f"/raid/scratch/data/hst/JOF/ACS_WFC/30mas/ACS_WFC_{band}_JOF_drz.fits"
        )
