...
class CLIInterface:
    ...
    def _update_display(self):
        ...
        with self.lock:
            ...
            top_line = ['ResolvedGalaxy CLI', f'Time elapsed: {time.time() - self.start_time:.2f}s', f'Computer: {computer}']
            ...
            for i, item in enumerate(top_line):
                self.screen.addstr(1, i * (width // len(top_line)), item[:width // len(top_line)-1], curses.color_pair(i))
            ...
            try:
                ...
                index = np.argwhere([i[0] == f'Core {core_num}' for i in psutil.sensors_temperatures()['coretemp']])
                if len(index) > 0:
                    # flatten index
                    index = index.flatten()
            ...
            for i, item in enumerate(info_line):
                self.screen.addstr(2, i * (width // len(info_line)), item[:width // len(info_line)-1], curses.color_pair(i))
            ...
            for i, line in enumerate(self.lines):
                for j, item in enumerate(line):
                    self.screen.addstr(i+4, j * (width // len(line)), item[:width // len(line)-1], curses.color_pair(i))
            
            # Show current task.
            if self.current_task is not None:
                self.screen.addstr(height-2, 0, f'Current task: {self.current_task}', curses.color_pair(3))
...
class ResolvedGalaxy:
    ...
    def __init__(self, galaxy_id : int, 
                ...
                resolved_mass = None, resolved_sfh = None, resolved_sed = None,
                h5_folder = 'galaxies/', 
    ...
    def init(cls, galaxy_id, survey, version, instruments = ['ACS_WFC', 'NIRCam'], 
                            ...
                            aper_diams = [0.32] * u.arcsec, output_flux_unit = u.uJy, h5_folder = 'galaxies/',
        ...
        if type(galaxy_id) not in [list, np.ndarray, Column]:
            ...
            if all(found):
                print('Loading from .h5')
                return cls.init_multiple_from_h5(galaxy_names, h5_folder = h5_folder)
        ...
        return cls.init_from_galfind(galaxy_id, survey, version, instruments = instruments, 
                          excl_bands = excl_bands, cutout_size=cutout_size, forced_phot_band = forced_phot_band, 
                            aper_diams = aper_diams, output_flux_unit = output_flux_unit, h5_folder = h5_folder,
    ...
    def init_from_galfind(cls, galaxy_id, survey, version, instruments = ['ACS_WFC', 'NIRCam'], 
        ...
        for position, gal_id in enumerate(galaxy_id):
            ...
            object = cls(galaxy_id = gal_id, sky_coord = galaxy_skycoord, survey = survey, bands = bands, 
                            ...
                            rms_err_paths = err_paths, rms_err_exts = rms_err_exts, im_pixel_scales = im_pixel_scales,
    ...
    @classmethod
    def init_all_field_from_h5(cls, field, h5_folder = 'galaxies/'):
    ...
    def init_from_h5(cls, h5_name, h5_folder = 'galaxies/', return_attr = False):
        ...
        with h5.File(h5path, 'r') as hfile:
            ...
            if 'auto_photometry' in hfile.keys():
                ap = hfile['auto_photometry']['auto_photometry'][()].decode('utf-8').replace('<Quantity', '').replace('>', '').replace('array([', '([').replace(', dtype=float64', '')
        ...
        out_dict = {'galaxy_id': galaxy_id, 'sky_coord': sky_coord, 'survey': survey, 'bands': bands, 'im_paths': im_paths,
                    ...
                    'seg_imgs': seg_imgs, 'aperture_dict': aperture_dict, 'psf_matched_data': psf_matched_data, 'galfind_version': galfind_version,
                    ...
                    'redshift': redshift, 'flux_map': flux_map, 'photometry_properties': photometry_properties, 'photometry_meta_properties': photometry_meta_properties,
                    'already_psf_matched': already_psf_matched, 'dont_psf_match_bands': dont_psf_match_bands, 'auto_photometry': auto_photometry,
                    'unmatched_data': unmatched_data, 'unmatched_rms_err': unmatched_rms_err, 'unmatched_seg': unmatched_seg, 'meta_properties': meta_properties,
                    'det_data': det_data, 'total_photometry': total_photometry, 'resolved_mass': resolved_mass, 'resolved_sfh': resolved_sfh, 'resolved_sed': resolved_sed,
                    'cutout_size': cutout_size, 'h5_folder': h5_folder, 'psf_kernels': psf_kernels}
    ...
    def dump_to_h5(self, h5_folder='galaxies/', mode='append'):
        ...
        with h5.File(new_h5_path, 'w') as hfile:
            ...
            if self.resolved_sfh is not None:
                ...
                for key in self.resolved_sfh.keys():
                    hfile['resolved_sfh'].create_dataset(key, data=self.resolved_sfh[key])
            
            # Add resolved SED
            if self.resolved_sed is not None:
                hfile.create_group('resolved_sed')
                for key in self.resolved_sed.keys():
                    hfile['resolved_sed'].create_dataset(key, data=self.resolved_sed[key])
            ...
            if type(self) == MockResolvedGalaxy:
                ...
                if self.noise_images is not None:
                    ...
                    for key in self.noise_images.keys():
                        hfile['mock_galaxy']['noise_images'].create_dataset(key, data=self.noise_images[key])
                if self.property_images is not None:
                    hfile['mock_galaxy'].create_group('property_images')
                    for key in self.property_images.keys():
                        hfile['mock_galaxy']['property_images'].create_dataset(key, data=self.property_images[key])
                ...
                if self.sfh is not None:
                    ...
                    for key in self.sfh.keys():
                        if type(self.sfh[key]) == np.ndarray:
                            hfile['mock_galaxy']['sfh'].create_dataset(key, data=self.sfh[key])
                        else:
                            hfile['mock_galaxy']['sfh'].create_group(key)
                            for key2 in self.sfh[key].keys():
                                hfile['mock_galaxy']['sfh'][key].create_dataset(key2, data=self.sfh[key][key2])
    ...
    def plot_cutouts(self, bands=None, psf_matched=False, save=False, psf_type=None, save_path=None, show=False, facecolor='white', fig=None):
        ...
        if fig is None:
            raise ValueError("A figure must be provided.")
        ...
        for i, (ax, band) in enumerate(zip(axes, bands)):
            ...
            ax.text(0.9, 0.9, band, color='w', transform=ax.transAxes, ha='right', va='top', 
                    fontsize=12, fontweight='bold', path_effects=[PathEffects.Stroke(linewidth=2, foreground='black'), 
                                                                PathEffects.Normal()])
            ax.set_xticks([])
            ax.set_yticks([])
    ...
    def plot_lupton_rgb(self, red = [], green = [], blue = [], q = 1, stretch = 1, use_psf_matched=False, override_psf_type = None, return_array = True, save = False, save_path = None, show = False, fig = None, ax = None, label_bands = False):
        ...
        if label_bands:
            ax.text(0.05, 0.95, f'{"+".join(red)}', color='red', transform=ax.transAxes, ha='left', va='top', fontsize=12, fontweight='bold', path_effects=[PathEffects.Stroke(linewidth=2, foreground='white'), PathEffects.Normal()])
            ax.text(0.05, 0.85, f'{"+".join(green)}', color='green', transform=ax.transAxes, ha='left', va='top', fontsize=12, fontweight='bold', path_effects=[PathEffects.Stroke(linewidth=2, foreground='white'), PathEffects.Normal()])
            ax.text(0.05, 0.75, f'{"+".join(blue)}', color='blue', transform=ax.transAxes, ha='left', va='top', fontsize=12, fontweight='bold', path_effects=[PathEffects.Stroke(linewidth=2, foreground='white'), PathEffects.Normal()])
    ...
    def pixedfit_processing(self, use_galfind_seg = True, seg_combine = ['F277W', 'F356W', 'F444W'], gal_region_use = 'pixedfit',
        ...
        meta_dict = {'stacked_bands':'+'.join(seg_combine), 'seg_type':seg_type, 'use_all_pixels':use_all_pixels, 'gal_region_use':gal_region_use}
    ...
    def plot_photometry_bins(self, binmap_type='pixedfit', bins_to_show='all', fig = None, ax = None,
                            wav_unit = u.um, flux_unit = u.uJy, label_individual = False,
                            save=False, save_path=None, show=True, facecolor='white', cmap_bins = 'nipy_spectral_r'):
        ...
        for i, rbin in enumerate(bins_to_show):
            ...
            for j, band in enumerate(self.bands):
                ...
                if flux_unit == u.ABmag:
                    ...
                    yerr = [np.atleast_1d(np.abs(err_low)), np.atleast_1d(np.abs(err_up))]
                ...
                ax.errorbar(wav.to(wav_unit), flux, yerr=yerr, fmt='o', color=color, label = label, markeredgecolor='black')
    ...
    def plot_filter_transmission(self, bands = 'all', fig = None, ax = None, label = True, wav_unit = u.um):
        ...
        ax.set_ylabel('T$_{\lambda}$')
        


    def plot_overview(self, figsize = (12, 8), bands_to_show = ['F814W', 'F115W', 'F200W', 'F335M', 'F444W'], show = True, save = False):
        # GridSpec

        fig = plt.figure(figsize=figsize, dpi=200, facecolor='white', constrained_layout=True)
        gs = fig.add_gridspec(3, 2, width_ratios=[1.9, 1], height_ratios=[1, 1, 1.5])
        ...
        # Plot SED
        self.plot_photometry_bins(ax=ax_sed, fig = fig_sed, show = False, bins_to_show = ['TOTAL_BIN', 'MAG_APER_TOTAL', '1'], flux_unit = u.ABmag, label_individual = True)
        ...
        self.plot_filter_transmission(bands = 'all', fig = fig_sed, ax = ax_filt, label = False, wav_unit = u.um)
        ...
        last_cutout_ax.text(0.9, 0.9, 'Det Seg' if self.det_data is not None else 'F444W Seg', color='w', transform=last_cutout_ax.transAxes, ha='right', va='top', 
                            fontsize=12, fontweight='bold', path_effects=[PathEffects.Stroke(linewidth=2, foreground='black'), 
                                                                        PathEffects.Normal()])
        ...
        self.plot_lupton_rgb(ax = ax_rgb, fig = fig_bins, show = False, red = ['F444W'], green = ['F356W'], blue = ['F200W'], return_array = False, stretch = 0.001, q = 0.001, label_bands = True)

        # plot pixedfit
        norm = ax_pixedfit.imshow(self.pixedfit_map, origin='lower', interpolation='none', cmap = 'nipy_spectral_r')  
        ...
        fig.text(0.05, 0.46, textstr, transform=fig.transFigure, fontsize=14, verticalalignment='top', bbox=props)
        textstr = f'RA: {self.sky_coord.ra.degree:.4f}\nDec: {self.sky_coord.dec.degree:.4f}\nCutout size: {self.cutout_size} pix'
        fig.text(0.5, 0.46, textstr, transform=fig.transFigure, fontsize=14, verticalalignment='top', bbox=props)

        if save:
            fig.savefig(f'galaxies/diagnostic_plots/{self.survey}_{self.galaxy_id}_overview.png', dpi=200)


        return fig

    def run_bagpipes(self, bagpipes_config, filt_dir = bagpipes_filter_dir, fit_photometry = 'all',
    run_dir = f'pipes/', overwrite=False, overwrite_internal = False):
        ...
        if type(redshift) == str:
            ...
            if redshift == 'eazy':
                ...
                if len(table) == 0:
                    raise Exception(f'No ID {redshift_id} found in {redshift} table when attempting to fetch redshift for bagpipes')
        ...
        if type(redshift) in [float, np.float64] and redshift_sigma is not None:
        ...
        if not exist_already:
            fit_cat = pipes.fit_catalogue(ids, fit_instructions, self.provide_bagpipes_phot,
                                        ...
                                        redshift_sigma = redshift_sigma, save_pdf_txts = False, 
    ...
    def convert_table_to_map(self, table, id_col, value_col, map, remove_log10=False):
        ...
        for row in table:
            ...
            try:
                ...
                continue
            value = row[value_col]
            return_map[map == float(gal_id)] = value if not remove_log10 else 10**value
    ...
    def plot_bagpipes_corner(self, run_name=None, bins_to_show = 'all', save=False, corner_bins=25, facecolor='white', colors='black', cache=None, fig = None,
        ...
        for rbin, color, in zip(bins_to_show, colors):
            h5_path = f'{run_dir}/posterior/{run_name}/{self.survey}/{self.galaxy_id}/{rbin}.h5'

            if rbin == 'RESOLVED':
                continue
            
            pipes_obj = self.load_pipes_object(run_name, rbin, run_dir = run_dir, cache=cache, plotpipes_dir=plotpipes_dir)
    ...
    def plot_bagpipes_fit(self, run_name=None, axes = None, fig=None, bins_to_show = 'all', save=False, 
                        ...
                        flux_units=u.ABmag, lw=1,fill_uncertainty=False,zorder=5, show_photometry = False, run_dir = f'pipes/', cache=None):
        ...
        if cache is None:
            cache = {}
        for rbin, color, in zip(bins_to_show, marker_colors):
            h5_path = f'{run_dir}/posterior/{run_name}/{self.survey}/{self.galaxy_id}/{rbin}.h5'
            ...
            if show_photometry:
                ...
                pipes_obj.plot_best_photometry(axes, colour=color, zorder=6, wav_units=wav_units, flux_units=flux_units)
    ...
    def plot_bagpipes_sfh(self, run_name=None, bins_to_show = 'RESOLVED', save=False, 
                        facecolor='white', marker_colors='black', time_unit='Gyr', cmap = 'viridis',
                        plotpipes_dir='pipes_scripts/', 
                        run_dir = f'pipes/', cache=None, fig=None, axes=None):
        sys.path.insert(1, plotpipes_dir)
            ...
            if rbin == 'RESOLVED':
                ...
                if self.resolved_sfh is not None:
                    if run_name in self.resolved_sfh.keys():
                        ...
                        axes.plot(x_all, resolved_sfh[:, 2], color='tomato', label='RESOLVED', lw = 2)
                        axes.fill_between(x_all, resolved_sfh[:, 1], resolved_sfh[:, 3], color='tomato', alpha=0.5)
                        continue
                    ...
                    for pos, tbin in enumerate(np.unique(table['#ID'])):
                        ...
                        try:
                            ...
                            tx, tsfh = pipes_obj.plot_sfh(dummy_ax, color, timescale=time_unit, plottype='lookback', logify=False, cosmo=None, label = rbin, return_sfh=True)
                            if pos == 0:
                                x_all = tx
                                y_all = tsfh
                            else:
                    ...
                    if set:
                        ...
                        self.add_to_h5(save_array, 'resolved_sfh', run_name, overwrite=True)
                ...
                pipes_obj = self.load_pipes_object(run_name, rbin, run_dir = run_dir, cache=cache, plotpipes_dir=plotpipes_dir)
                pipes_obj.plot_sfh(axes, color, modify_ax = True, add_zaxis=True, timescale=time_unit, plottype='lookback', logify=False, cosmo=None, label = rbin)
    ...
    def load_pipes_object(self, run_name, rbin, run_dir = f'pipes/', bagpipes_filter_dir=bagpipes_filter_dir, cache=None, plotpipes_dir='pipes_scripts/', get_advanced_quantities = True, n_cores = 1):
    ...
    def plot_bagpipes_component_comparison(self, parameter = 'stellar_mass', run_name=None, 
                                        ...
                                        cache = None, colors = None, fig = None, axes = None):
    ...
    def get_total_resolved_mass(self, run_name, sed_fitting_tool = 'bagpipes', n_draws = 10000, return_quantiles = True, run_dir = 'pipes/', pipes_dir = 'pipes_scripts/', overwrite = False):
        table = self.sed_fitting_table[sed_fitting_tool][run_name]
        bins = []
        bins_to_show = table['#ID']

        if return_quantiles:
            if self.resolved_mass is not None and run_name in self.resolved_mass.keys() and not overwrite:
        ...
        pipes_objs = self.load_pipes_object(run_name, bins, get_advanced_quantities=True, run_dir = run_dir, plotpipes_dir=pipes_dir)
    ...
    def get_resolved_bagpipes_sed(self, run_name, run_dir = f'pipes/', plotpipes_dir = 'pipes_scripts/', overwrite = False):
        ...
        for pos, galaxy_id in enumerate(count):
            pipes_obj = self.load_pipes_object(run_name, galaxy_id, run_dir = run_dir, cache=None, plotpipes_dir=plotpipes_dir)
            # Get SED

            wav, flux = pipes_obj.plot_best_fit(None, wav_units=u.um, flux_units=u.uJy, return_flux = True)
            ...
            if pos == 0:
                ...
                assert all(wav == total_wav), f'Wavelengths do not match, {np.min(wav)} - {np.max(wav)} vs {np.min(total_wav)} - {np.max(total_wav)}'
    ...
    def plot_bagpipes_sed(self, run_name, run_dir = f'pipes/', bins_to_show='all', plotpipes_dir = 'pipes_scripts/'):
        ...
        for pos, rbin in enumerate(bins_to_show):
            print('rbin', rbin)
            if rbin == 'RESOLVED':
                total_flux, total_wav = self.get_resolved_bagpipes_sed(run_name, run_dir = run_dir, plotpipes_dir = plotpipes_dir)
                ax_sed.plot(total_wav, total_flux, label='Resolved', color='black')
                continue

            else:
                pipes_obj = self.load_pipes_object(run_name, rbin, run_dir = run_dir, cache=None, plotpipes_dir=plotpipes_dir)
                ...
                pipes_obj.plot_best_fit(ax_sed, colour=color[str(rbin)],  wav_units=u.um, flux_units=u.ABmag, lw=1, fill_uncertainty=False, zorder=5, linestyle = '-.' if pos in other_count else 'solid', label = rbin if pos in other_count else "")
                # Plot photometry
                pipes_obj.plot_sed(ax_sed, colour=color[str(rbin)], wav_units=u.um, flux_units=u.ABmag, zorder = 6, fcolour=color[str(rbin)], ptsize=15)
                try:
                    float(rbin)
                    ...
                    y = np.mean(y)
                    x = np.mean(x)
                    ax_map.text(x, y, rbin, fontsize=8, color='black', path_effects=[PathEffects.withStroke(linewidth=1, foreground='white')])
                except ValueError:
                    pass
        # Set x-axis limits
    ...
    def galfind_phot_property_map(self, property, iters = 100, load_in = True, density = False, logmap = False, plot = True, ax = None, facecolor = 'white', cmap='viridis', **kwargs):
        ...
        for pos, gal_id in enumerate(tqdm(np.unique(map))):
            ...
            phot = copy.deepcopy(self.galfind_photometry_rest[str(int(gal_id))])
            loaded = False
...
class MockResolvedGalaxy(ResolvedGalaxy):
    def __init__(self, galaxy_id, mock_survey, mock_version, instruments = ['ACS_WFC', 'NIRCam'], bands = [],
                            excl_bands = [], cutout_size = 64, forced_phot_band = ["F277W", "F356W", "F444W"], 
                            aper_diams = [0.32] * u.arcsec, output_flux_unit = u.uJy, h5_folder = 'galaxies/',
                            dont_psf_match_bands=[], already_psf_matched = False,
                            sky_coord = None, im_paths = [], im_exts = [], err_paths = [], rms_err_exts = [], 
                            seg_paths = [], rms_err_paths = [], phot_img_headers = {},
                            psf_matched_data = None, psf_matched_rms_err = None,    
                            phot_imgs = {}, rms_err_imgs = {}, seg_imgs = {}, aperture_dict = None, redshift = None,
                            im_pixel_scales = None, im_zps = None, phot_pix_unit = None, det_data = None, 
                            synthesizer_galaxy = None, noise_images = None, meta_properties = None, 
                            property_images = None, auto_photometry = None, seds = None, sfh=None, **kwargs):
        ...
        super().__init__(galaxy_id = galaxy_id, sky_coord = sky_coord, survey = mock_survey, bands = bands, 
                            im_paths = im_paths, im_exts = im_exts, im_zps = im_zps, seg_paths = seg_paths,
                            detection_band = '+'.join(forced_phot_band), galfind_version = mock_version,
                            rms_err_paths = err_paths, rms_err_exts = rms_err_exts, im_pixel_scales = im_pixel_scales,
                            phot_imgs = phot_imgs, phot_pix_unit = phot_pix_unit, phot_img_headers = phot_img_headers, 
                            rms_err_imgs = rms_err_imgs, seg_imgs = seg_imgs, aperture_dict = aperture_dict, redshift = redshift,
                            cutout_size = cutout_size, dont_psf_match_bands = dont_psf_match_bands, auto_photometry = auto_photometry,
                            psf_matched_data = psf_matched_data, psf_matched_rms_err = psf_matched_rms_err, meta_properties = meta_properties,
                            already_psf_matched = already_psf_matched, det_data = det_data, overwrite=True, **kwargs)
    ...
    def init_from_h5(cls, h5_name, h5_folder = 'galaxies/', return_attr = False):
        ...
        with h5.File(h5path, 'r') as hfile:
            ...
            if 'meta_properties' in mock_galaxy.keys():
                params['meta_properties'] = ast.literal_eval(mock_galaxy['meta_properties'][()].decode('utf-8'))
        ...
        variables = super().init_from_h5(h5_name = h5_name, h5_folder = h5_folder, return_attr = True)
    ...
    @classmethod
    def init_mock_from_synthesizer(cls, redshift_code, galaxy_index,
                                    ...
                                    grid_dir = "./grids/",
                                    debug = False,
                                    depth_file = {'NIRCam':'/raid/scratch/work/austind/GALFIND_WORK/Depths/NIRCam/v11/JOF/0.32as/n_nearest/JOF_depths.ecsv',
        ...
        if update_cli_interface:
            ...
            lines = [[f'Galaxy: {redshift_code}_{galaxy_index}', f'Survey: {mock_survey}', f'Version: {mock_version}'],
                                [f'Synthesizer: Grid: {grid_name}'],
                                [''], ['']]

            lines[2][0] = f'Current Task: Initializing Mock Galaxy from Synthesizer'
            cli.update(lines)
        ...
        if False:
            ...
            # Load catalogue and populate galaxies
            cat = Catalogue.from_pipeline(survey = mock_survey, version = mock_version, instruments = mock_instruments,
        ...
        try:
            ...
            from unyt import Hz, erg, Mpc, kpc, Msun, yr, s, arcsecond, nJy, K, uJy, unyt_array, um, Angstrom, Myr, Gyr
        ...
        grid = Grid(grid_name, grid_dir=grid_dir) # TODO: Apply reasonable wavelength limits to grid to reduce computation time and memory usage

        #tags = ['010_z005p000', '008_z007p000', '007_z008p000', '005_z010p000']
        #regions = [['00', '00', '01', '10', '18'], ['00', '02', '09'], ['21', '17'], ['15']]
        #ids = [[12, 96, 1424, 1006, 233], [6, 46, 298], [111, 16], [99]]
        filter_codes = [f'{obs}/{inst}.{band}' for obs, inst, band in zip(observatories, instruments, bands)]
        ...
        with h5.File(file_path,'r') as hf:  #opening the hdf5 file
            ...
            coordinates = np.array(hf[f"{tag}/{region}/{gal_id}"].get('coordinates'), dtype=np.float64) * Mpc
            # initial masses of the stellar particles
            initial_masses = np.array(hf[f"{tag}/{region}/{gal_id}"].get('initial_masses'), dtype=np.float64) * Msun
            # current masses of the stellar particles, mass change due to mass loss as stars age
            current_masses = np.array(hf[f"{tag}/{region}/{gal_id}"].get('current_masses'), dtype=np.float64) * Msun
            # ages of stars in log10, which is in units of years
            log10ages = np.array(hf[f"{tag}/{region}/{gal_id}"].get('log10ages'), dtype=np.float64)
            # metallicities of the stars in log10
            log10metallicities = np.array(hf[f"{tag}/{region}/{gal_id}"].get('log10metallicities'), dtype=np.float64)
            ...
            smoothing_lengths = np.array(hf[f"{tag}/{region}/{gal_id}"].get('smoothing_lengths'), dtype=np.float64) * Mpc
        ...
        model_assumptions = {'gridname': grid_name,
                            'grid_dir': grid_dir,
                            'file_path': file_path,
                            'dust_type': 'pacman',
        ...
        emission_model = PacmanEmission(grid=grid,
                                        tau_v=tau_v,
        ...
        if cutout_size == 'auto':
            ...
            half_light_radius = np.max([gal.stars.get_flux_radius('total', band, flux_enclosed).to(kpc) for band in size_bands]) * kpc

            # convert to arcseconds
            half_light_radius = half_light_radius.to_astropy()
            d_A = cosmo.angular_diameter_distance(gal.redshift)
            half_light_radius_arcsec = (half_light_radius / d_A).to(u.arcsec, u.dimensionless_angles())
        ...
        property_images = {}
        for prop in ['stellar_mass', 'sfr', 'ssfr', 'stellar_age', 'stellar_metallicity']:
        ...
        radius_kpc = (mock_aper_diams[0] * d_A).to(u.kpc, u.dimensionless_angles())
        radius_kpc = radius_kpc.value/2 * kpc

        noise_app_imgs = psf_imgs.apply_noise_from_snrs(snrs=snrs, depths=depths, aperture_radius=radius_kpc)
        ...
        possible_galaxies = glob.glob(f'galaxies/{mock_survey}_*.h5')
        ...
        if update_cli_interface:
            cli.update(current_task = f'Finished approximating RMS error maps.', progress = 0)
        ...
        for band in bands:
            ...
            if debug:
                fig, ax = plt.subplots(nrows=1, ncols=3, figsize=(10, 5), constrained_layout=True, facecolor='white')
                #one = ax[0].imshow(noise_images[band], origin='lower', cmap='viridis')
                one = ax[0].imshow(noise_app_imgs[filter_code_from_band[band]].arr/rms_err_images[band], origin='lower', cmap='viridis')
        ...
        if update_cli_interface:
            cli.update(current_task = f'Running SEP to get segmentation maps and measured fluxes.')

        # Our convolution kernel (gauss_2.5_5x5.conv) is:

        conv = np.array([[0.034673, 0.119131, 0.179633, 0.119131, 0.034673],
        ...
        if update_cli_interface:
            cli.update(current_task = f'Stacking {detection_band} images to get detection image.')


        inv_var_weights = {band: 1 / rms_err_images[band]**2 for band in mock_forced_phot_band}
    
        # Calculate the weighted sum of images
        weighted_sum = np.sum([final_images[band] * inv_var_weights[band] for band in mock_forced_phot_band], axis=0)
        
        # Calculate the sum of weights
        sum_of_weights = np.sum([inv_var_weights[band] for band in mock_forced_phot_band], axis=0)
        ...
        if debug:
            plt.imshow(detection_image/detection_rms_error, origin='lower', cmap='viridis')
            plt.colorbar()
            plt.show()
            ...
            plt.imshow(final_images[mock_forced_phot_band[0]], origin='lower', cmap='viridis', norm=LogNorm())
            plt.title(f'{mock_forced_phot_band[0]}')
            plt.show()

        if update_cli_interface:
            cli.update(current_task = f'Running sep.extract()')


        detection_objects, detection_segmap = sep.extract(detection_image, thresh = 1.8, err = detection_rms_error, 
                            minarea = 9,
                            deblend_nthresh = 32,
                            filter_kernel = conv,
                            deblend_cont = 0.005,    
        ...
        detect_seg_id = detection_segmap[int(detection_segmap.shape[0]/2), int(detection_segmap.shape[1]/2)]

        # Plot the detection image and objects
        #if debug:
        fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(10, 5), constrained_layout=True, facecolor='white')
        ax[0].imshow(detection_image, origin='lower', cmap='viridis', norm=LogNorm())
                
        for i in range(len(detection_objects)):
                e = Ellipse(xy=(detection_objects['x'][i], detection_objects['y'][i]),
                width=6*detection_objects['a'][i],
                height=6*detection_objects['b'][i],
                angle=detection_objects['theta'][i] * 180. / np.pi)
        ...
        for band in bands:
            if update_cli_interface:
                cli.update(current_task = f'Running detection and segmentation for {band}')

            band_objects, segmap = sep.extract(final_images[band], thresh = 1.8, err = rms_err_images[band],
                            minarea = 9,
                            deblend_nthresh = 32,
                            filter_kernel = conv,
                            deblend_cont = 0.005,
            ...
            flux, fluxerr, flag = sep.sum_circle(final_images[band], detection_objects['x'], detection_objects['y'],
            pixel_radius, err=rms_err_images[band], gain=1.0)
            ...
            aper_depths.append(-2.5 * np.log10(5*depths[filter_code_from_band[band]]) + 23.9)
            ...
            if update_cli_interface:
                cli.update(current_task = f'Doing aperture photometry and kron fluxes for {band}')


            kronrad, krflag = sep.kron_radius(final_images[band], band_objects['x'], band_objects['y'], band_objects['a'], band_objects['b'], band_objects['theta'], 6.0)
            kron_flux, kron_fluxerr, kron_flag = sep.sum_ellipse(final_images[band], band_objects['x'], band_objects['y'], band_objects['a'], band_objects['b'], band_objects['theta'], 2.5*kronrad, subpix=1)
            ...
            cflux, cfluxerr, cflag = sep.sum_circle(final_images[band], band_objects['x'][use_circle], band_objects['y'][use_circle],
            r_min, subpix=1)
            ...
            r, flag = sep.flux_radius(final_images[band], band_objects['x'], band_objects['y'], 6.*band_objects['a'], 0.5,
            normflux=kron_flux, subpix=5)
            ...
            if seg_id != 0:
                ...
                kron_flux = kron_flux[obj_mask] if type(kron_flux) == np.ndarray else kron_flux
                kron_fluxerr = kron_fluxerr[obj_mask] if type(kron_fluxerr) == np.ndarray else kron_fluxerr
                ...
                auto_photometry[band]['MAG_AUTO'] = -2.5 * np.log10(np.atleast_1d(kron_flux)) + 23.9 # 23.9 is the zero point for a uJy image
                auto_photometry[band]['MAGERR_AUTO'] = np.atleast_1d(kron_fluxerr)/np.atleast_1d(kron_flux) * 2.5/np.log(10)
        ...
        aperture_dict = {str(0.32*u.arcsec): {'flux': flux_aper, 'flux_err': flux_err_aper, 'depths': aper_depths, 'wave': wave}} 
        ...
        meta_properties = {'redshift':gal.redshift,
                            'id':gal_id,
                            'tag':tag,
                            'region':region,
                            'grid':grid_name,
        ...
        if update_cli_interface:
                cli.update(current_task = f'Saving Synthesizer SEDs.')
        ...
        if update_cli_interface:
            message = f'Finished generating mock galaxy.'
            cli.update(current_task = message)
            cli.stop()



        return cls(galaxy_id, mock_survey, mock_version, instruments = mock_instruments, bands = bands,
                    excl_bands = [], cutout_size = cutout_size, forced_phot_band = mock_forced_phot_band, 
                    aper_diams = mock_aper_diams, output_flux_unit = u.uJy, h5_folder = 'galaxies/',
                    dont_psf_match_bands=[], already_psf_matched = False, synthesizer_galaxy = gal,
                    aperture_dict = aperture_dict, auto_photometry = auto_photometry, redshift = zed,
                    rms_err_imgs = rms_err_images, seg_imgs = seg_imgs, psf_matched_data = None, psf_matched_rms_err = None,
    ...
    @classmethod
    def init(cls, mock_survey = None, redshift_code = None, galaxy_index = None,  galaxy_id = None, overwrite = False, h5_folder = 'galaxies/', **kwargs):
        # load from h5 if it exists
        # Initialize CLI if relevant

        assert (mock_survey is not None) and (redshift_code is not None) and (galaxy_index is not None) or (galaxy_id is not None), 'Need to provide either galaxy_id or mock_survey, redshift_code, and galaxy_index'
        ...
        if os.path.exists(file_path) and not overwrite:
            print(f'Loading from .h5')
            return cls.init_from_h5(galaxy_id, h5_folder = h5_folder)
        else:
            print(f'Generating from synthesizer.')
            return cls.init_mock_from_synthesizer(redshift_code, galaxy_index, mock_survey = mock_survey, **kwargs)
    ...
    def plot_mock_spectra(self, components, fig = None, ax = None, facecolor = 'white', wav_unit = u.um, flux_unit = u.uJy, label = True, **kwargs):
        if fig is None:
    ...
    def plot_property_maps(self, parameters=['stellar_mass', 'sfr_10 Myr', 'ssfr_10 Myr', 'stellar_age', 'stellar_metallicity'], save=False, facecolor='white', max_on_row=4, weight_mass_sfr = True, norm =  'linear', logmap = [True, True, True, True, False], total_params = []):
        ...
        for i, param in enumerate(parameters):
            ...
            mappable = axes[i].imshow(map, origin='lower', interpolation='none', cmap=cmaps[i], norm=anorm)
            cbar = fig.colorbar(mappable, cax=cax, orientation='horizontal')
    ...
    def regenerate_synthesizer_galaxy(self, file_path = '/nvme/scratch/work/tharvey/synthesizer/flares_flags_balmer_project.hdf5', basic = False):
        ...
        grid = Grid(grid_name = self.meta_properties['grid'], grid_dir = "/nvme/scratch/work/tharvey/synthesizer/grids/")
        ...
        if filter_codes is None:
            print('No filter codes found in meta_properties. Using default filters.')
            observatories = 5*['HST'] + 14 * ['JWST'] 
            instruments = 5*['ACS_WFC'] + 14 * ['NIRCam']
            bands = self.bands
        ...
        with h5.File(file_path,'r') as hf:  #opening the hdf5 file
            ...
            initial_masses = np.array(hf[f"{tag}/{region}/{gal_id}"].get('initial_masses'), dtype=np.float64) * Msun
            # current masses of the stellar particles, mass change due to mass loss as stars age
            current_masses = np.array(hf[f"{tag}/{region}/{gal_id}"].get('initial_masses'), dtype=np.float64) * Msun
            # ages of stars in log10, which is in units of years
            log10ages = np.array(hf[f"{tag}/{region}/{gal_id}"].get('log10ages'), dtype=np.float64)
            # metallicities of the stars in log10
            log10metallicities = np.array(hf[f"{tag}/{region}/{gal_id}"].get('log10metallicities'), dtype=np.float64)
        ...
        if not basic:
            ...
            if 'dust_emission' in model_assumptions.keys():
                if model_assumptions['dust_emission'] == 'greybody':
                    dust_emission = Greybody(model_assumptions['dust_temp'] * K, model_assumptions['dust_beta'])
            ...
            emission_model = dust_object(grid=grid,
                                        tau_v=gal.tau_v,
    ...
    def plot_bagpipes_sfh(self, run_name=None, bins_to_show = 'RESOLVED', save=False, 
                        facecolor='white', marker_colors='black', time_unit='Gyr', cmap = 'viridis',
                        plotpipes_dir='pipes_scripts/',
                        run_dir = f'pipes/', cache=None, fig=None, axes=None, add_true = True):

        # Call parent method

        fig, cache = super().plot_bagpipes_sfh(run_name = run_name, bins_to_show = bins_to_show, save = save, 
                                                facecolor = facecolor, marker_colors = marker_colors, 
                                                time_unit = time_unit, cmap = cmap, plotpipes_dir = plotpipes_dir, 
                                                run_dir = run_dir, cache = cache, fig = fig, axes = axes)
        if add_true:
            ...
            if len(axes) == 1:
                axes = axes[0]
                # Plot the true SFH
                self.plot_real_sfh(mask = {'attr':'gal_region', 'key':'pixedfit'}, ax = axes, fig = fig, time_unit = time_unit, mask_str = 'pixedfit')

        return fig, cache

    def plot_real_sfh(self, mask = None, ax = None, fig = None, mask_str = None, time_unit = 'Gyr', overwrite = False):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if contained_class:
            for method_name in dir(contained_class):
                if not method_name.startswith('__') and isinstance(getattr(contained_class, method_name), types.FunctionType):
                    setattr(new_class, method_name, create_forwarding_method(method_name))
...
class MultipleResolvedGalaxy:
    ...
    def init(cls, list_of_ids, survey, version, **kwargs):
        ...
        for gal_id in list_of_ids:
            if 'mock' in gal_id:
                galaxy = MockResolvedGalaxy.init(galaxy_id = gal_id, mock_survey = survey, mock_version = version, **kwargs)
            else:
                galaxy = ResolvedGalaxy.init(galaxy_id = gal_id, survey = survey, version = version, **kwargs)
    ...
    def mass_comparison_plot(self, aperture_run, resolved_run, sed_fitting_tool ='bagpipes', aperture_label = 'TOTAL_BIN', label = False, **kwargs):
        fig, ax = plt.subplots(1, 1, figsize=(5, 5), facecolor='white', dpi = 200)
        
        for galaxy in self.galaxies:
            table = galaxy.sed_fitting_table[sed_fitting_tool][aperture_run]
            table = table[table['#ID'] == aperture_label]
            assert len(table) == 1, f'Aperture label {aperture_label} not found in table or table is not unique: length {len(table)}'

            mass_16, mass_50, mass_84 = table['stellar_mass_16'], table['stellar_mass_50'], table['stellar_mass_84']

            # resolved mass

            quantities = galaxy.get_total_resolved_mass(resolved_run, sed_fitting_tool = sed_fitting_tool)

            ax.errorbar(mass_50, quantities[1], xerr = (mass_50 - mass_16, mass_84 - mass_50), yerr = ([quantities[1] - quantities[0]], [quantities[2] - quantities[1]]), fmt='o', label = galaxy.galaxy_id if label else '', **kwargs)

        ax.set_xlabel('Aperture Stellar Mass (M$_\odot$)')

        ax.set_ylabel('Resolved Stellar Mass (M$_\odot$)')
...
def run_bagpipes_wrapper(galaxy_id, resolved_dict, cutout_size, h5_folder, field = 'JOF_psfmatched', version = 'v11', overwrite = False, overwrite_internal = False):
        #print('Doing', galaxy_id, resolved_dict['meta']['run_name'], overwrite, overwrite_internal)
        # return
        
        try:
            if 'mock' in galaxy_id:
                galaxy = MockResolvedGalaxy.init(galaxy_id = galaxy_id, mock_survey = field, mock_version = version, cutout_size = cutout_size, h5_folder = h5_folder)
            else:
                galaxy = ResolvedGalaxy.init(galaxy_id, field, version, cutout_size = cutout_size, h5_folder = h5_folder)

            # Run bagpipes
            galaxy.run_bagpipes(resolved_dict, overwrite = overwrite, overwrite_internal = overwrite_internal)
            ...
            if resolved_dict['meta']['fit_photometry'] in ['bin', 'all']:
                ...
                fig, _ = galaxy.plot_bagpipes_sfh(resolved_dict['meta']['run_name'], bins_to_show=['RESOLVED'])
                plt.close(fig)
...
if __name__ == "__main__":
    ...
    galaxies = ResolvedGalaxy.init(list(ids), 'JOF_psfmatched', 'v11', already_psf_matched = True, cutout_size = cutout_size, h5_folder = h5_folder)
    # Should speed it up?
    
    cat = None
    num_of_bins = 0
    initial_load = True # Set to True if you want to load the data from the catalogue
    ...
    continuity_dicts = create_dicts(continuity_dict, override_meta = override_meta)
    ...
    for run_dicts in [delayed_dicts, continuity_dicts, dpl_dicts, lognorm_dicts]:
        size = 1
        if size >= 2:
        ...
        if n_jobs == 1:
            ...
            #    with parallel_config(backend=backend, n_jobs=n_jobs): # inner_max_num_threads=1 if backend is 'multiprocessing' else None)
            #        Parallel()(run_bagpipes)(run_dicts[i]) for i in range(len(galaxies))
