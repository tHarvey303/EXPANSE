...
def handle_map_click(
    ...
    cmap,
    which_map_param,
    which_sed_fitter_param,
    which_flux_unit_param,
    multi_choice_bins_param,
    which_run_aperture_param,
    which_run_resolved_param,
    total_fit_options_param,
    param_property,
...
def plot_rgb(resolved_galaxy, red, green, blue, scale, q, psf_mode):
    ...
    rgb = resolved_galaxy.plot_lupton_rgb(
        red, green, blue, scale, q, use_psf_matched=use_psf_matched
    )
    ...
    scale_bar_size = (
        resolved_galaxy.im_pixel_scales[any_band].to(u.arcsec).value
    )  # 1 pixel in arcsec
...
def plot_sed(
    ...
    if table is not None:
        for bin_pos, rbin in enumerate(multi_choice_bins_param + total_fit_options_param):
            ...
            if len(table_row) == 0:
                print(
                    f"No SED bins for {rbin}, psf_type {psf_type} and which_map_param {which_map_param}"
                )
                continue
    ...
    if which_sed_fitter_param == "bagpipes" and (
        ...
        # Plot resolved fits
        if (
            hasattr(resolved_galaxy, "sed_fitting_table")
            and "bagpipes" in resolved_galaxy.sed_fitting_table.keys()
            and which_run_resolved_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
        ):
            ...
            (
                fig,
                cache
                == resolved_galaxy.plot_bagpipes_fit(
                    which_run_resolved_param,
                    ax,
                    fig,
                    bins_to_show=multi_choice_bins_param,
        ...
        if (
            hasattr(resolved_galaxy, "sed_fitting_table")
            and "bagpipes" in resolved_galaxy.sed_fitting_table.keys()
            and which_run_aperture_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
        ):
            if resolved_galaxy.galaxy_id not in cache_pipes.keys():
                cache_pipes[resolved_galaxy.galaxy_id] = {}
    ...
    if which_sed_fitter_param == "bagpipes" and (
        ...
        if (
            ...
            fig, cache = resolved_galaxy.plot_bagpipes_component_comparison(
                ...
                run_name=which_run_resolved_param,
                bins_to_show=multi_choice_bins_param,
                save=False,
                run_dir=run_dir,
                facecolor=facecolor,
...
def plot_sfh(
    ...
    if (
        ...
        and "bagpipes" in resolved_galaxy.sed_fitting_table.keys()
        and (
            which_run_aperture_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
            or which_run_resolved_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
        )
    ):
        if which_sed_fitter_param == "bagpipes":
            ...
            if (
                which_run_aperture_param not in cache_pipes[resolved_galaxy.galaxy_id].keys()
                and which_run_aperture_param is not None
            ):
                cache_pipes[resolved_galaxy.galaxy_id][which_run_aperture_param] = {}

            if (
                which_run_resolved_param not in cache_pipes[resolved_galaxy.galaxy_id].keys()
...
def plot_corner(
    ...
    if (
        ...
        and "bagpipes" in resolved_galaxy.sed_fitting_table.keys()
        and (
            which_run_resolved_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
            or which_run_aperture_param in resolved_galaxy.sed_fitting_table["bagpipes"].keys()
        )
    ):
        if which_sed_fitter_param == "bagpipes":
            if resolved_galaxy.galaxy_id not in cache_pipes.keys():
            ...
            if which_run_aperture_param is not None:
                bins_to_show = total_fit_options_param
                cache = cache_pipes[resolved_galaxy.galaxy_id].get(which_run_aperture_param)
                fig, cache = resolved_galaxy.plot_bagpipes_corner(
                    run_name=which_run_aperture_param,
                    bins_to_show=bins_to_show,
                    save=False,
                    facecolor=facecolor,
...
def do_snr_plot(band):
    return pn.pane.Matplotlib(
        resolved_galaxy.plot_snr_map(band=band, facecolor=facecolor),
        dpi=144,
        tight=True,
        format="svg",
        width=300,
...
def plot_phot_property(
    property,
    cmap,
    strong_line_names=["Hbeta", "[OIII]-4959", "[OIII]-5007", "Halpha"],
    ...
    fig = resolved_galaxy.galfind_phot_property_map(
...
def plot_map_with_controls(map_array, label="", unit=""):
    ...
    colormap_choice = pn.widgets.Select(options=plt.colormaps(), value="viridis", name="Colormap")
...
def synthesizer_page():
    ...
    fig = pn.pane.Matplotlib(
        resolved_galaxy.plot_mock_spectra(components=["det_segmap_fnu"], facecolor=facecolor),
        dpi=144,
        tight=True,
        format="svg",
        sizing_mode="stretch_both",
...
def phot_prop_func():
    ...
    # strong_line_options =
    drop_down_4 = pn.widgets.Select(
        name="Line",
        options=["Hbeta", "[OIII]-4959", "[OIII]-5007", "Halpha"],
        width=100,
        value="Hbeta",
    )
    line_plot1 = pn.param.ParamFunction(
        pn.bind(
            plot_phot_property,
            "line_flux_rest_optical",
            "viridis",
            drop_down_4.param.value,
            watch=False,
        ),
        loading_indicator=True,
    )
    middle_row.append(pn.Column(line_plot1, sizing_mode="stretch_width"))
    line_plot2 = pn.param.ParamFunction(
        pn.bind(
            plot_phot_property,
            "EW_rest_optical",
            "viridis",
...
@cached_function
def other_bagpipes_results_func(
    param_property,
    p_run_name,
    norm_param,
    total_params,
    ...
    if outtype == "gif":
        ...
        fig = resolved_galaxy.plot_bagpipes_results(
            facecolor=facecolor,
...
def sed_results_plot_func(which_sed_fitter_param, which_run_resolved_param, upscale_select_param):
    if which_sed_fitter_param == "bagpipes":
        sed_results_plot = resolved_galaxy.plot_bagpipes_results(
            facecolor=facecolor,
            parameters=["stellar_mass", "sfr"],
            max_on_row=3,
            run_name=which_run_resolved_param,
            norm="linear",
            weight_mass_sfr=upscale_select_param,
        )
...
def handle_file_upload(value, components):
    ...
    which_map = pn.widgets.RadioButtonGroup(
        options=["pixedfit", "voronoi"], value="pixedfit", name="Pixel Binning"
    )

    which_sed_fitter = pn.widgets.Select(name="SED Fitter", value="bagpipes", options=["bagpipes"])

    which_flux_unit = pn.widgets.Select(
        name="Flux Unit", value="uJy", options=["uJy", "ABmag", "ergscma"]
    )
    ...
    which_run_aperture = pn.widgets.Select(
        name="Integrated SED Fitting Run", value=None, options=[]
    )
    ...
    choose_show_band = pn.widgets.Select(
        name="Show Band", options=resolved_galaxy.bands, value="F444W"
    )
    ...
    row = pn.Row(
        pn.bind(plot_cutouts, psf_mode_select.param.value, "star_stack"),
        sizing_mode="stretch_both",
        scroll=False,
    )
    ...
    bin_map = pn.bind(
        ...
        "pixedfit",
        cmap,
        scale_alpha.param.value,
        show_galaxy.param.value,
        show_kron.param.value,
        psf_mode_select.param.value,
        choose_show_band.param.value,
    )
    ...
    show_sed_photometry = pn.widgets.Select(
        name="Fit Photometry",
    ...
    sed_obj = pn.param.ParamFunction(
        pn.bind(
            handle_map_click,
            stream.param.x,
            stream.param.y,
            resolved_galaxy,
            cmap,
            which_map.param.value,
            which_sed_fitter.param.value,
            which_flux_unit.param.value,
            multi_choice_bins.param.value,
            which_run_aperture.param.value,
            which_run_resolved.param.value,
            total_fit_options.param.value,
            None,
            show_sed_photometry.param.value,
            "sed",
            watch=False,
        ),
        loading_indicator=True,
    ...
    sfh_obj = pn.param.ParamFunction(
        pn.bind(
            handle_map_click,
            stream.param.x,
            stream.param.y,
    ...
    corner_obj = pn.param.ParamFunction(
        pn.bind(
            handle_map_click,
            stream.param.x,
            stream.param.y,
    ...
    pdf_obj = pn.param.ParamFunction(
        pn.bind(
            handle_map_click,
            stream.param.x,
            stream.param.y,
            ...
            which_run_resolved.param.value,
            total_fit_options.param.value,
            pdf_param_property.param.value,
            None,
            "pdf",
            watch=False,
        ),
    ...
    red_select = pn.widgets.MultiChoice(
        name="Red Channel", options=resolved_galaxy.bands, value=["F444W"]
    )
    green_select = pn.widgets.MultiChoice(
        name="Green Channel", options=resolved_galaxy.bands, value=["F277W"]
    )
    blue_select = pn.widgets.MultiChoice(
        name="Blue Channel", options=resolved_galaxy.bands, value=["F150W"]
    )

    stretch_slider = pn.widgets.EditableFloatSlider(
        name="Stretch", start=0.001, end=10, value=6, step=0.001
    )
    q_slider = pn.widgets.EditableFloatSlider(
        name="Q", start=0.000001, end=0.01, value=0.001, step=0.000001
    )
    ...
    other_plot_select = pn.widgets.Select(
        options=["Galaxy Region", "Fluxes", "Segmentation Map", "Radial SNR"],
        value="Galaxy Region",
    )
        ...
        pn.Row(
            pn.Column(
                ...
                pn.bind(
                    plot_rgb,
                    resolved_galaxy,
                    red_select.param.value,
                    green_select.param.value,
                    blue_select.param.value,
                    stretch_slider.param.value,
                    q_slider.param.value,
                    psf_mode_select.param.value,
                ),
                scroll=False,
            ),
            pn.Column(band_select, snr_plot, scroll=False),
            sizing_mode="scale_both",
            max_width=1000,
            scroll=False,
            min_height=500,
    ...
    cutout_grid.append(
        pn.Row(
            pn.Column(
                "### Other Plots",
                pn.Row(other_plot_select, scroll=False),
                pn.Row(other_plot, min_height=500, scroll=False),
            ),
            sizing_mode="scale_both",
            max_width=1000,
            scroll=False,
    ...
    """
    upscale_select = pn.widgets.Select(
        name="Upscale Mass/SFR", options=[True, False], value=True, width=100
    )
    ...
    sed_results = pn.bind(
        sed_results_plot_func,
        which_sed_fitter.param.value,
        which_run_resolved.param.value,
        upscale_select.param.value,
        watch=True,
    )

    other_bagpipes_properties_dropdown = pn.widgets.Select(
        name="SED Properties Map",
        options=["constant:age_max"],
        value="constant:age_max",
    )
    type_select = pn.widgets.Select(
        name="Type", options=["median", "gif"], value="median", width=90
    )
    ...
    other_bagpipes_results = pn.param.ParamFunction(
        pn.bind(
            other_bagpipes_results_func,
            other_bagpipes_properties_dropdown.param.value,
            which_run_resolved.param.value,
            norm.param.value,
            total_fit_options.param.value,
            upscale_select.param.value,
            type_select.param.value,
            watch=False,
        ),
        loading_indicator=True,
    )
    ...
    bot_row = pn.Row(
        corner_obj,
        pn.Column(
            pn.Row(other_bagpipes_properties_dropdown),
            pn.Row(norm, type_select),
            pn.Row(other_bagpipes_results),
        ),
        height=750,
    )
    ...
    empty_phot_page = pn.Column(
        loading=True, sizing_mode="stretch_both", min_height=400, min_width=400
    )  # , max_width=1000)
    # phot_prop = pn.param.ParamFunction(phot_prop_func,  lazy=True)

    galaxy_tabs = pn.Tabs(
        ("Cutouts", cutout_grid),
        ("SED Results", combined),
        ("Photometric Properties", empty_phot_page),
        dynamic=True,
        scroll=False,
    )
    ...
    try:
        ...
        print(f"Requested tab {initial_tab} not recognized. Defaulting to Cutouts.")
        tab_index = 0
...
def plot_options():
    ...
    accordion = pn.Accordion(
        ...
        (
            "X-Scale",
            pn.widgets.RadioButtonGroup(options=["linear", "log"], value="linear"),
        ),
        (
            "Y-Scale",
            pn.widgets.RadioButtonGroup(options=["linear", "log"], value="linear"),
...
def resolved_sed_interface():
    ...
    pn.bind(
        handle_file_upload, file_input, components, watch=True
