...
from mpl_toolkits.axes_grid1.anchored_artists import AnchoredSizeBar
from astropy.visualization import make_lupton_rgb, LogStretch, ManualInterval, ImageNormalize, PercentileInterval, AsymmetricPercentileInterval, ZScaleInterval, LinearStretch
# Bye warnings
...
class PipesFit:
    def __init__(self, galaxy_id, field, h5_path, pipes_path, catalog, overall_field=None,
        ...
        if self.catalog is not None:
            if len(self.catalog) > 1:
                raise Exception(f'{len(self.catalog)} matches for {galaxy_id} in column {ID_col}')
            try:
        ...
        self.stellar_mass_16, self.stellar_mass_50, self.stellar_mass_84 = np.percentile(self.fit.posterior.samples["stellar_mass"], (16, 50, 84))
        ...
        sfh = [x.split(':')[0]  for x in self.fit.fitted_model.params if x.split(':')[-1] == 'massformed'][0]
        ...
        fiducial = {'sfh':'lognormal', 'dust_type': 'Calzetti', 'dust_prior':r'$\mathrm{log_{10}}$', 'metallicity_prior':r'$\mathrm{log_{10}}$', 'age_prior': r'$\mathrm{log_{10}}$', 'redshift': 'zgauss', 'fesc':0.0, 'sps':'BC03'}
        show = []
    ...
    def plot_csfh(self, ax, colour='black', modify_ax = True, add_zaxis=True, timescale='Gyr', plottype='lookback', logify=False, cosmo=None, **kwargs):
        ...
        if modify_ax:
            if plottype == 'lookback':
                ax.set_xlabel(f"$\\mathbf{{\\mathrm{{Lookback\\ Time \\ ({timescale})}}}}$", fontsize='small',  path_effects=[pe.withStroke(linewidth=2, foreground='white')])

            elif plottype == 'absolute':
                ax.set_xlabel(f"$\\mathbf{{\\mathrm{{Age\\ of\\ Universe \\ ({timescale})}}}}$", fontsize='small', patheffects=[pe.withStroke(linewidth=2, foreground='white')])
    ...
    def plot_sfh(self, ax, colour='black', modify_ax = True, add_zaxis=True, timescale='Myr', plottype='lookback', logify=False, cosmo=None, return_sfh = False, **kwargs):
        ...
        if modify_ax:
            ...
            ax.set_ylabel("$\\mathrm{SFR\\ (M_\\odot\\ \\mathrm{yr}^{-1}})$", fontsize='medium')
    ...
    def plot_best_photometry(self, ax, colour='black', wav_units=u.um, flux_units=u.ABmag, zorder=4, y_scale=None,
                             skip_no_obs=False, background_spectrum=False, **kwargs):  
        ...
        if self.fitted_type in ['phot', 'both']:
            ...
            wavs = fit.posterior.model_galaxy.wavelengths*(1.+redshift) *u.AA 
            ...
            if background_spectrum:
                spec_post = np.percentile(fit.posterior.samples["spectrum_full"],
                                        (16, 84), axis=0).T * u.erg/(u.cm**2 * u.s * u.AA)
                ...
                ax.plot(wavs_plot, flux_nu[:, 0], color="black",
                        zorder=zorder-1)

                ax.plot(wavs_plot, flux_nu[:, 1], color="black",
                        zorder=zorder-1)

                ax.fill_between(wavs_plot, flux_nu[:, 0], flux_nu[:, 1],
            ...
            for j in range(fit.galaxy.photometry.shape[0]):
                ...
                phot_band = fit.posterior.samples["photometry"][:, j] * u.erg/(u.cm**2 * u.s * u.AA)
                ...
                if len(phot_1sig) > 0:
                        ax.scatter(wav_array, phot_1sig, color=colour, zorder=zorder, 
    ...
    def plot_sed(self, ax, colour='black', wav_units=u.um, flux_units=u.ABmag, x_ticks=None, zorder=4, ptsize=40,
                            ...
                            label=None,  marker="o", rerun_fluxes=False, **kwargs):
        ...
        if self.fitted_type in ['phot', 'both']:
            ...
            if rerun_fluxes:
                fluxes, bands =  run_bagpipes.load_fits(self.galaxy_id, return_both=True, verbose=False, input_unit = self.catalogue_flux_unit)
            ...
            try:
                for pos, band in enumerate(self.bands):
                    ...
                    if (sigma[0][0] < 2):
                        ...
                        three_sig_depth = useful_funcs.five_sig_depth_to_n_sig_depth(loc_depth, 3)*u.ABmag
                        three_sig_depth = three_sig_depth.to(flux_units).value
                        wav_pos = wav[pos].to(wav_units).value
                        p1 = patches.FancyArrowPatch((wav_pos, three_sig_depth), (wav_pos, three_sig_depth+0.5), arrowstyle='-|>', mutation_scale=10, alpha=1, color=colour)
                        ax.add_patch(p1)   
            ...
            ax.errorbar(wav_lim, plot_fnu,
                        yerr=[err_low, err_up], lw=lw,
                        linestyle=" ", capsize=3, capthick=lw, zorder=zorder-1,
                        color=colour)

            ax.scatter(wav_lim, plot_fnu, color=colour,
                    s=ptsize, zorder=zorder, linewidth=lw, facecolor=fcolour,
                    edgecolor=colour, marker=marker, label=label) 
...
class PlotPipes:
    ...
    match_field=True, match_version=False, catalog_path='', 
    spectra_path='', catalog_version=None, exclude_folders=[], 
    ID_col='NUMBER', field_col='field', ra_col='ALPHA_J2000', 
    dec_col = 'DELTA_J2000',redshift_col='zbest', robust_col=None,
     mixed_cat=False, overall_field=None,radius_col='FLUX_RADIUS_F277W+F356W+F444W',
      eazy_template='fsps_larson', add_prospector=False, compact_plot=True,
    prospector_run_name='continuity_flex_dynesty', catalogue_flux_unit=u.MJy/u.sr, simulated_cat=False):
        ...
        exclude_folders.extend([f'{self.overall_field}/plot_temp', f'{self.overall_field}/temp'])
        ...
        try:
            ...
            self.row = Table(self.catalog[(self.catalog[self.id_col] == num_id) & mask == True])
            
            if len(self.row) == 0:
                raise Exception(f'No row found for {self.galaxy_id} in {self.catalog_path}')
            elif len(self.row) > 1:
                raise Exception(f'Multiple matches found for {self.galaxy_id} in {self.catalog_path}')
        except FileNotFoundError:
        ...
        for h5_file in self.h5_paths:
            #print('Doing ', h5_file)
            try:
                fit_obj = PipesFit(self.galaxy_id, self.field, h5_file, self.pipes_path, self.row, load_spectrum=False, overall_field=self.overall_field, field_col=self.field_col, ID_col=self.id_col, catalogue_flux_unit=self.catalogue_flux_unit)
    ...
    def annotate_nearby(self, ax, size, pixel_scale):
        ...
        if length < 2000:
            print('This catalogue seems short. If only robust galaxies are included, the nearby annotations won\'t work.')
        center  = SkyCoord(self.row[self.ra_col][0]*u.degree, self.row[self.dec_col][0]*u.degree, unit='deg') 
        catalog_sky = SkyCoord(ra=self.catalog[self.ra_col]*u.degree, dec = self.catalog[self.dec_col]*u.degree)
        for i in range(2, 10):
            ...
            if d3d != 0.0:
                ...
                if d2d[0] < max_sep:
                    ...
                    wcs = self.cutout_wcs
                    coords = wcs.world_to_pixel(SkyCoord(match[self.ra_col]*u.degree, match[self.dec_col]*u.degree))
                    color = 'green' if match[f'{self.robust_col}_{self.eazy_template}'] else 'white'
                    ax.text(coords[0], coords[1]-10, f"{match[self.id_col]}:z~{match[self.redshift_col+'_'+self.eazy_template]:.1f}", color=color,ha='center', va='center', fontsize='small')
                    radius = match[self.radius_col]
                    region = patches.Circle((coords[0], coords[1]), radius, fill=False, linestyle='--', lw=1, color=color, zorder=20)
                    ax.add_patch(region)
    
    def jaguar_cat(self, ax_photo, ax_z_pdf, ax_sfr, ax_metallicity, ax_mass_pdf, ax_ssfr, ax_dust):
        ...
        ax_z_pdf.vlines(true_redshift, ax_z_pdf.get_ylim()[0], ax_z_pdf.get_ylim()[1], color='black', linestyle='--', lw=1, zorder=10)
        
        ax_sfr.set_ylim(ax_sfr.get_ylim()[0], ax_sfr.get_ylim()[1])
        ax_sfr.vlines(true_sfr, ax_sfr.get_ylim()[0], ax_sfr.get_ylim()[1], color='black', linestyle='--', lw=1, zorder=10)

        ax_metallicity.set_ylim(ax_metallicity.get_ylim()[0], ax_metallicity.get_ylim()[1])
        ax_metallicity.vlines(true_Z, ax_metallicity.get_ylim()[0], ax_metallicity.get_ylim()[1], color='black', linestyle='--', lw=1, zorder=10)

        ax_mass_pdf.set_ylim(ax_mass_pdf.get_ylim()[0], ax_mass_pdf.get_ylim()[1])
        ax_mass_pdf.vlines(true_mass, ax_mass_pdf.get_ylim()[0], ax_mass_pdf.get_ylim()[1], color='black', linestyle='--', lw=1, zorder=10)
        ...
        ax_dust.vlines(true_dust, ax_dust.get_ylim()[0], ax_dust.get_ylim()[1], color='black', linestyle='--', lw=1, zorder=10)

    def pretty_plot_condensed(self, cmap = flexoki_colors, wav_units = u.um, add_cutouts=True, add_sfh=True, flux_units=u.ABmag, max_bands=8):
        if type(cmap) == str:
        ...
        if bands is not None:
            if len(bands) > max_bands:
                ...
                height_ratios = [2.5 - height_bands/4, 1.]

        subfigs = fig.subfigures(2, 1, wspace=0.07, hspace=0.07, height_ratios=height_ratios)   
        top_fig = subfigs[0]
        ...
        ax_sfh = inset_axes(ax_photo, width="30%", height="30%", loc='upper left', bbox_to_anchor=(-0.013,0.020,1,1), bbox_transform=ax_photo.transAxes) # bbox_to_anchor=(0.10,-0.005,1,1)
        ...
        if add_cutouts:
            if bands is not None:
                ...
                for pos, band in tqdm(enumerate(bands), desc='Adding cutouts'):
                    if height_bands > 1:
                        cutout_ax = cutout_fig.add_subplot(cutout_gs[pos//width, pos%width])
                    else:
                        cutout_ax = cutout_fig.add_subplot(cutout_gs[0, pos])
                    
                    if self.plot_cutout(cutout_ax, band, show_scale=0.5 if band == 'F444W' else False):
                        countdown +=1
                        ...
                        cutout_ax.set_ylabel(band, fontsize='medium', color=color, fontweight='bold')
                        cutout_ax.set_xticks([])
                        cutout_ax.set_yticks([])
                        try:

                            path_effects=[pe.withStroke(linewidth=4, foreground="black")]
                            cutout_ax.annotate(f"{self.row[f'sigma_{band}'][0][0]:.1f}$\sigma$", xy=(0.94, 0.82),ha='right', xycoords='axes fraction', color='white', path_effects=path_effects)
                        except: 
                            pass
                    else:    
                        cutout_ax.remove()
        ...
        ax_photo.set_xlabel(f'Wavelength ({wav_units})',  fontsize='medium', fontweight='bold')
        ...
        top_fig.suptitle(f'{self.field}:{self.galaxy_id.split("_")[0]}', fontsize='large', fontweight='bold')
        ...
        for pos, fit in enumerate(self.fits):
            ...
            fit.plot_best_fit(ax_photo, colour=colours[pos], zorder=3, wav_units=wav_units, flux_units=flux_units, fill_uncertainty=fill_uncertainty)
            # Plot best photometry
            fit.plot_best_photometry(ax_photo, colour=colours[pos], zorder=6, wav_units=wav_units, flux_units=flux_units)
            ...
            if True:
                ...
                try:
                    ...
                    pass
                fit.plot_pdf(ax_mass_pdf, 'stellar_mass', colour=colours[pos], fill_between=fill_uncertainty)
        ...
        ax_mass_pdf.set_xlabel(r'$ \mathrm{\bf Stellar \ Mass } \ (\log_{10}\mathrm{M}_{*}/\mathrm{M}_{\odot})$', fontsize='medium', fontweight='bold')
        ...
        ax_sfh.set_yticklabels([0.01, 1, 100], path_effects=[pe.withStroke(linewidth=2, foreground="white")])
        ...
        try:
            fig.savefig(f'{bagpipes_dir}/pipes/plots/{self.overall_field}/{self.galaxy_id}_compact.png', dpi=200, bbox_inches='tight')
            fig.savefig(f'{bagpipes_dir}/pipes/plots/merged/{self.galaxy_id}_compact.png', dpi=200, bbox_inches='tight')
    ...
    def pretty_plot(self, cmap = flexoki_colors, wav_units = u.um, add_rgb =True, add_posterior=True, add_cutouts=True, add_sfh=True, flux_units=u.ABmag, size=150, pixel_scale=0.03, blue=['F090W', 'F115W','F150W'], green=['F200W', 'F277W'], red=['F356W', 'F410M','F444W']):
        # cmap='cmr.ember'
        ...
        if add_cutouts:
            if bands is not None:
                ...
                for pos, band in tqdm(enumerate(bands), desc='Adding cutouts'):
                    ...
                    cutout_ax = fig.add_subplot(gs[int(np.floor(3+0.2*countdown)), count:count+2])
                    ...
                    if self.plot_cutout(cutout_ax, band):
                        ...
                        color = 'blue' if band in blue else 'green' if band in green else 'red' if band in red else 'black'
                        ...
                        try:
                            ...
                            path_effects=[pe.withStroke(linewidth=4, foreground="black")]
                            cutout_ax.annotate(f"{self.row[f'sigma_{band}'][0][0]:.1f}$\sigma$", xy=(0.94, 0.85),ha='right', xycoords='axes fraction', color='white', path_effects=path_effects)
                        except: 
                            pass
                    else:    
                        cutout_ax.remove()
        ...
        try:
            if add_rgb:
                ...
                ax_color.set_xticks([])
                ax_color.set_yticks([])
                added = self.plot_colour(ax_color, size=size, pixel_scale=pixel_scale, blue=blue, green=green, red=red)
                if added:
                    print('Adding RGB')
        ...
        ax_photo.set_title(f'{self.overall_field}:{self.galaxy_id.split("_")[0]}', fontsize='large')
        ...
        for pos, fit in enumerate(self.fits):
            ...
            if True:
                fit.plot_sfh(ax_sfh, colour=colours[pos], add_zaxis=True if pos == 0 else False, modify_ax=True if pos == 0 else False, logify=False, timescale='Myr', plottype='absolute')
            if add_posterior:
                print('Adding posterior PDFs')
                try:
                    fit.plot_pdf(ax_z_pdf,'redshift', colour=colours[pos], fill_between=fill_uncertainty)
                    ...
                    pass
                fit.plot_pdf(ax_mass_pdf, 'stellar_mass', colour=colours[pos], fill_between=fill_uncertainty)
                fit.plot_pdf(ax_metallicity, 'metallicity', colour=colours[pos], fill_between=fill_uncertainty)
                fit.plot_pdf(ax_dust, 'dust:Av', colour=colours[pos], fill_between=fill_uncertainty)
                fit.plot_pdf(ax_sfr, 'sfr', colour=colours[pos], fill_between=fill_uncertainty)
                fit.plot_pdf(ax_ssfr, 'ssfr', colour=colours[pos], fill_between=fill_uncertainty)
                # This is broken for some reason - think 
                fit.plot_pdf(ax_nebular, 'nebular:logU', colour=colours[pos], fill_between=fill_uncertainty)
        ...
        if self.simulated_cat:
            self.jaguar_cat(ax_photo, ax_z_pdf, ax_sfr, ax_metallicity, ax_mass_pdf, ax_ssfr, ax_dust)
        # Add legend
        ...
        if not Path(f'{bagpipes_dir}/pipes/plots/{self.overall_field}/').is_dir:
            os.mkdir(f'{bagpipes_dir}/pipes/plots/{self.overall_field}/')
        try:
            fig.savefig(f'{bagpipes_dir}/pipes/plots/{self.overall_field}/{self.galaxy_id}.png', dpi=200, bbox_inches='tight')
        except:
                #Sometimes redshift axis breaks, this attempts to remove and resave
                ax_sfh.cla()
    ...
    def plot_colour(self, ax, ra_col='ALPHA_J2000', dec_col='DELTA_J2000', pixel_scale=0.03, size=150, blue=['F115W', 'F150W'], green=['F200W', 'F277W'], red=['F356W','F444W']):
        path = {band:self.get_image_path(band, self.field) for band in blue+green+red}
        ...
        if red_data is not None and green_data is not None and blue_data is not None:
            ...
            if not skip:
                ...
                rgb = make_lupton_rgb(cutout_r.data, cutout_g.data, cutout_b.data, Q=1, stretch=0.05)
                #with np.printoptions(threshold=np.inf):
                #    print(rgb)
                im=ax.imshow(rgb, origin='lower', interpolation='none')
                scalebar = AnchoredSizeBar(ax.transData,
                            1/pixel_scale, "1\"", 'lower right', 
                            pad=0.3,
                            color='white',
                            frameon=False,
                            size_vertical=2,
                ...
                region = patches.Circle((xpos, ypos), 0.32/pixel_scale, fill=False, linestyle='--', lw=1, color='white')
                ax.add_patch(region)
                return True
        else:
            print('Data is None')
    ...
    def plot_cutout(self, ax, band,
        ...
        if found_image:
            ...
            if not skip:
                ...
                ax.imshow(data_cutout, norm=norm if not use_orig else None, cmap=cmap,origin=origin )
                
                xpos = np.mean(ax.get_xlim())
                ypos = np.mean(ax.get_ylim())
                region = patches.Circle((xpos, ypos), radius_pix, fill=False, linestyle='--', lw=1, color=linecolor, zorder=zorder)
                ax.add_patch(region)

                if show_scale:
                    scalebar = AnchoredSizeBar(ax.transData,
                            0.5/pixel_scale, "0.5\"", 'lower right', 
                            pad=0.1,
                            color='white',
                            frameon=False,
                            size_vertical=1,
    ...
    def add_prospector(self, run_name='continuity_flex_dynesty', prospect_dir = prospector_dir):
...
def main(id, field):
    ...
    if field in ['JADES-Deep-GS', 'NEP-1', 'NEP-2', 'NEP-3', 'NEP-4', 'NGDEEP', 'MACS-0416'] or field.startswith('CEERSP'):
    ...
    PlotPipes(f'{id}_{field}', field, catalog_path=catalog_path,
overall_field='CEERS' if field.startswith('CEERSP') else None, robust_col='final_sample_highz', eazy_template='fsps_larson', simulated_cat=False, add_prospector=False,
    prospector_run_name=['continuity_bursty_dynesty_zgauss', 'continuity_bursty_dynesty_hot_imf_zgauss',  'delayed_dynesty_zgauss',  'delayed_dynesty_hot_imf_zgauss'],
    exclude_folders=['sfh_continuity_bursty_dust_Cal_uniform_Z_log_10_age_log_10_bpass_fesc_zgauss', 
